...
require_once 'header_after_pat.php';
?>
...
<body>
  ...
  if (isset($_POST['predict'])) {
    ...
    $command = escapeshellcmd("python3 diabetes_prediction_d1.py --pregnancies $pregnancies --glucose $glucose --blood_pressure $blood_pressure --skin_thickness $skin_thickness --insulin $insulin --bmi $bmi --diabetes_pedigree $diabetes_pedigree --age $age");
    $output = (int)shell_exec($command);

    $query = "INSERT INTO prediction (pid, pregnancies, glucose, blood_pressure, skin_thickness, insulin, bmi, diabetes_pedigree, age, prediction) 
    VALUES ('$pid', '$pregnancies', '$glucose', '$blood_pressure', '$skin_thickness', '$insulin', '$bmi', '$diabetes_pedigree', '$age', '$output')";
    ...
    if ($result) {
      if ($output == 1) {
        ...
        &nbsp; &nbsp;<a style='text-decoration:none;' href='aa.php' target='_blank'><button class='dr_consult'>Consult a Doctor</button></a>";
        echo '</div>';
      } else {
      ...
      echo "Error storing prediction data: " . mysqli_error($connection);
    }
  }
  ?>

<form action="" method="post" class="animate__animated animate__fadeIn" style="max-width: 600px; margin: 0 auto;">
    <div style="display: flex; flex-wrap: wrap; justify-content: space-between;">
      <!-- Left Side -->
      <div style="flex: 1; margin-right: 10px;">
        <label for="pregnancies">Number of Pregnancies (times):</label>
      ...
      <div style="flex: 1; margin-left: 10px;">
        <label for="insulin">Insulin Level (muU/ml):</label>

...
<body>
    ...
    if (isset($_POST['delete_apt'])) {
        $aid = $_POST['aid'];
        $result = mysqli_query($connection, "UPDATE appointments SET status = 'CLOSED' WHERE aid = '$aid'");
        if ($result) {
    ...
    $did = $_POST['did'];
    $pname = $_POST['pname'];
    $resultApt = mysqli_query($connection, "SELECT * FROM appointments app JOIN doctorprofile dr ON app.did = dr.did JOIN vitals v ON app.aid = v.aid WHERE app.aid = $aid");

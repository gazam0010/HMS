...
if (isset($_POST['unset_session'])) {
    ...
    $query1 = mysqli_query($connection, "SELECT * from appointments WHERE aid='$aid'");
...
$resultAptEng = mysqli_query($connection, "SELECT * FROM appointments app JOIN doctorprofile d ON d.did = app.did JOIN patient p ON app.pid = p.pid JOIN vitals v ON app.aid = v.aid WHERE app.aid = $aid");
...
if (isset($_POST['other_action'])) {
    ...
    $updateStatus = mysqli_query($connection, "UPDATE appointments SET status = '$other_action' WHERE aid = $aid");
    if ($updateStatus) {
...
if (isset($_POST['complete_apt'])) {
    ...
    $updateStatus = mysqli_query($connection, "UPDATE appointments SET status = 'COMPLETED', comm = '$comment' WHERE aid = $aid");
    if ($updateStatus) {
